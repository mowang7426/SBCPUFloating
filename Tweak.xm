...
static void updateFloatingSize()
{
    ...
    if(autoWindowSizeEnable)
    {
        ...
        CGSize targetSize = CGSizeMake(textSize.width + 44, textSize.height + 26);

        if(targetSize.width < 125) targetSize.width = 125;
        if(targetSize.height < 64) targetSize.height = 64;
    ...
    else
    {
        ...
        CGSize targetSize = landscape ? CGSizeMake(165 * scale, 70 * scale) : CGSizeMake(135 * scale, 64 * scale);
