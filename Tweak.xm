...
static void LoadPreferences(void) {
    ...
    if ([processName isEqualToString:@"SpringBoard"]) {
        applyVisibility();
        if (showFps || force120HzEnable) [[SBCPUFPSHelper sharedInstance] startMonitoring];
...
static NSString *getNetworkType(void) {
    ...
    if (cell) {
        @try {
            static id networkInfo = nil; static dispatch_once_t onceToken;
            dispatch_once(&onceToken, ^{ Class CTClass = NSClassFromString(@"CTTelephonyNetworkInfo"); if (CTClass) networkInfo = [[CTClass alloc] init]; });
            if (networkInfo) {
                NSString *tech = nil;
                if ([networkInfo respondsToSelector:@selector(dataServiceIdentifier)] && [networkInfo respondsToSelector:@selector(serviceCurrentRadioAccessTechnology)]) {
                    ...
                    if (dataServiceId && [dict isKindOfClass:[NSDictionary class]]) tech = dict[dataServiceId];
                } 
                if (!tech && [networkInfo respondsToSelector:@selector(serviceCurrentRadioAccessTechnology)]) {
                    NSDictionary *dict = [networkInfo valueForKey:@"serviceCurrentRadioAccessTechnology"];
                    if ([dict isKindOfClass:[NSDictionary class]] && dict.allValues.count > 0) tech = dict.allValues.firstObject;
                }
                if (!tech && [networkInfo respondsToSelector:@selector(currentRadioAccessTechnology)]) tech = [networkInfo valueForKey:@"currentRadioAccessTechnology"];
                if (tech) {
                    ...
                    if ([tech containsString:@"CDMA"] || [tech containsString:@"WCDMA"] || [tech containsString:@"HSDPA"]) return @"3G 网络";
                    return @"2G 网络";
                }
            }
        } @catch (NSException *e) {}
    ...
    if (service) {
        ...
        if (IORegistryEntryCreateCFProperties(service, &prop, kCFAllocatorDefault, 0) == KERN_SUCCESS && prop) {
            ...
            if (pDict[@"AdapterDetails"]) {
                dict[@"Watts"] = pDict[@"AdapterDetails"][@"Watts"];
                dict[@"ChargerType"] = pDict[@"AdapterDetails"][@"Description"];
            }
            CFRelease(prop);
        }
        IOObjectRelease(service);
...
static BOOL isDeviceOverheated(void) {
    if (@available(iOS 11.0, *)) {
        ...
        if (state == NSProcessInfoThermalStateSerious || state == NSProcessInfoThermalStateCritical) return YES;
    }
    if (getBatteryTemperatureInternal() >= 43.0) return YES;
    return NO;
}
...
static double getSystemCPUUsage(void) {
    ...
    if (!has_prev_cpu_load) {
        prev_cpu_load = cpu_load; has_prev_cpu_load = YES;
        return 12.0;
    }
...
static double getRealCPUFrequency(void) {
    ...
    return getDeviceSpec().maxFreqMHz;
}
...
static void clampAndPositionFloatingView(CGPoint targetCenter, BOOL animate) {
    ...
    CGFloat halfW = floatingView.frame.size.width / 2.0f, halfH = floatingView.frame.size.height / 2.0f;
    CGFloat minX = halfW + 4.0f, maxX = containerBounds.size.width - halfW - 4.0f;
    CGFloat minY = halfH + 20.0f, maxY = containerBounds.size.height - halfH - 10.0f;
    ...
    if (floatingView.isCollapsed) targetCenter.x = (targetCenter.x <= containerBounds.size.width / 2.0f) ? minX : maxX;
        ...
        CGFloat distLeft = targetCenter.x - halfW, distRight = containerBounds.size.width - (targetCenter.x + halfW);
...
static void updateFloatingSize(void) {
    ...
    CGFloat rot = 0.0;
    switch (getActiveInterfaceOrientation()) {
        case UIInterfaceOrientationLandscapeLeft: rot = -M_PI_2; break;
        case UIInterfaceOrientationLandscapeRight: rot = M_PI_2; break;
        case UIInterfaceOrientationPortraitUpsideDown: rot = M_PI; break;
        default: break;
    }
    floatingView.transform = CGAffineTransformConcat(CGAffineTransformMakeScale(floatingScale, floatingScale), CGAffineTransformMakeRotation(rot));
    clampAndPositionFloatingView(floatingView.center, NO);
}

// ✅ [核心修复] - iOS 15 强制 Window 绑定生命周期健康的 Scene
static void createCPUWindow(UIWindowScene *scene) {
    if (cpuWindow) return;
    if (!scene) return;

    cpuWindow = [[SBCPUWindow alloc] initWithFrame:UIScreen.mainScreen.bounds];
    cpuWindow.windowScene = scene; // iOS 15 关键点：绑定活跃的 Scene
    cpuWindow.windowLevel = UIWindowLevelStatusBar + 1500.0; // 极高层级，解决遮挡问题
...
static void updateCPU(void) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (smartChargeLimitEnable && temp > 0) {
            if (temp >= smartChargeLimitTemp && !isCurrentlyChargeInhibited) { setHardwareChargingInhibit(YES); isCurrentlyChargeInhibited = YES; } 
            else if (temp <= (smartChargeLimitTemp - 1.5f) && isCurrentlyChargeInhibited) { setHardwareChargingInhibit(NO); isCurrentlyChargeInhibited = NO; }
...
+ (instancetype)sharedInstance { static SBCPUFPSHelper *i = nil; static dispatch_once_t o; dispatch_once(&o, ^{ i = [[SBCPUFPSHelper alloc] init]; }); return i; }
...
- (void)startDriverAnimation {
    ...
    a.fromValue = @(0.01f); a.toValue = @(0.02f); a.duration = 1.0; a.repeatCount = HUGE_VALF; a.autoreverses = YES; a.removedOnCompletion = NO;
    if (@available(iOS 15.0, *)) a.preferredFrameRateRange = CAFrameRateRangeMake(120.0f, 120.0f, 120.0f);
    [_driverLayer addAnimation:a forKey:@"ProMotion120Driver"];
}
...
- (void)updateFrameRate {
    ...
    if (@available(iOS 15.0, *)) {
        ...
        if (apply120) {
            if ([_displayLink respondsToSelector:@selector(setHighFrameRateReason:)]) { @try { [_displayLink setValue:@(1114113) forKey:@"highFrameRateReason"]; } @catch (id ex) {} }
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        UITapGestureRecognizer *dtap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleDoubleTap:)]; dtap.numberOfTapsRequired = 2; dtap.delegate = self; [self addGestureRecognizer:dtap]; [_singleTapGesture requireGestureRecognizerToFail:dtap];
        ...
        _blurView.layer.cornerRadius = 20.0f; _blurView.layer.masksToBounds = YES; _blurView.layer.borderWidth = 0.75f; _blurView.layer.borderColor = [UIColor colorWithWhite:1.0f alpha:0.30f].CGColor; _blurView.userInteractionEnabled = NO; [self addSubview:_blurView];
...
- (void)collapseToEdgeAnimated:(BOOL)animated {
    ...
    CGFloat targetW = 64.0f, targetH = 28.0f, targetHalfW = targetW / 2.0f, targetHalfH = targetH / 2.0f;
    ...
    CGFloat minY = targetHalfH + 20.0f, maxY = containerBounds.size.height - targetHalfH - 10.0f;
    CGFloat targetY = MIN(MAX(self.center.y, minY), maxY); CGPoint targetCenter = CGPointMake(targetX, targetY);
    self.collapsedContainerView.hidden = NO;

    void (^animationsBlock)(void) = ^{
        self.cpuTitleLabel.alpha = 0; self.cpuValueLabel.alpha = 0; self.cpuFreqLabel.alpha = 0; self.div1.alpha = 0;
        self.fpsValueLabel.alpha = 0; self.fpsSubLabel.alpha = 0; self.divFps.alpha = 0;
        self.batteryIconLabel.alpha = 0; self.batteryValueLabel.alpha = 0; self.batterySubLabel.alpha = 0; self.div2.alpha = 0;
        self.tempIconLabel.alpha = 0; self.tempValueLabel.alpha = 0; self.tempSubLabel.alpha = 0; self.div3.alpha = 0;
        self.currentIconLabel.alpha = 0; self.currentValueLabel.alpha = 0; self.currentSubLabel.alpha = 0; self.bottomCapsule.alpha = 0;
...
- (void)expandFromEdgeAnimated:(BOOL)animated {
    ...
    _isCollapsed = NO; BOOL charging = isChargingInternal();
    UIView *parent = self.superview; CGRect containerBounds = parent ? parent.bounds : [UIScreen mainScreen].bounds;
    ...
    CGFloat expandedW = self.bounds.size.width, expandedH = self.bounds.size.height, expandedHalfW = expandedW / 2.0f, expandedHalfH = expandedH / 2.0f;
    BOOL isLeft = (self.center.x <= containerBounds.size.width / 2.0f);
    CGFloat targetX = isLeft ? (expandedHalfW + 4.0f) : (containerBounds.size.width - expandedHalfW - 4.0f);
    CGFloat minY = expandedHalfH + 20.0f, maxY = containerBounds.size.height - expandedHalfH - 10.0f;
    CGFloat targetY = MIN(MAX(self.center.y, minY), maxY); CGPoint targetCenter = CGPointMake(targetX, targetY);

    void (^animationsBlock)(void) = ^{
        self.collapsedContainerView.alpha = 0.0;
        self.cpuTitleLabel.alpha = 1; self.cpuValueLabel.alpha = 1; self.cpuFreqLabel.alpha = 1; self.div1.alpha = 1;
        self.fpsValueLabel.alpha = 1; self.fpsSubLabel.alpha = 1; self.divFps.alpha = 1;
        self.batteryIconLabel.alpha = 1; self.batteryValueLabel.alpha = 1; self.batterySubLabel.alpha = 1; self.div2.alpha = 1;
        self.tempIconLabel.alpha = 1; self.tempValueLabel.alpha = 1; self.tempSubLabel.alpha = 1; self.div3.alpha = 1;
        self.currentIconLabel.alpha = 1; self.currentValueLabel.alpha = 1; self.currentSubLabel.alpha = 1; self.bottomCapsule.alpha = 1;
...
- (void)handlePan:(UIPanGestureRecognizer *)pan {
    ...
    if (pan.state == UIGestureRecognizerStateBegan) { if (_isCollapsed) [self expandFromEdgeAnimated:NO]; self.lastPoint = self.center; }
        ...
        UIView *parent = self.superview; CGRect containerBounds = parent ? parent.bounds : [UIScreen mainScreen].bounds;
        CGFloat halfW = self.frame.size.width / 2.0f, halfH = self.frame.size.height / 2.0f;
        CGFloat minX = halfW + 2.0f, maxX = containerBounds.size.width - halfW - 2.0f;
        CGFloat minY = halfH + 20.0f, maxY = containerBounds.size.height - halfH - 10.0f;
...
- (void)updateLayoutWithShowCpuFreq:(BOOL)showFreq showFps:(BOOL)showFps showBatteryPercent:(BOOL)showBattery showBatteryTemp:(BOOL)showTemp showBatteryCurrent:(BOOL)showCurrent isCharging:(BOOL)isCharging {
    ...
    _currentIconLabel.hidden = !actualShowCurrent; _currentValueLabel.hidden = !actualShowCurrent; _currentSubLabel.hidden = !actualShowCurrent; _bottomCapsule.hidden = !isCharging;

    CGFloat currentX = 10.0f, padY = 8.0f, cpuW = 68.0f;
    ...
    if (showFps) {
        CGFloat fpsW = 28.0f; _fpsValueLabel.frame = CGRectMake(currentX, padY, fpsW, 14); _fpsSubLabel.frame = CGRectMake(currentX, padY + 14, fpsW, 11); currentX += fpsW + 6.0f;
    ...
    if (showBattery) {
        CGFloat batW = 48.0f; _batteryIconLabel.frame = CGRectMake(currentX, padY + 3, 16, 22); _batteryValueLabel.frame = CGRectMake(currentX + 18, padY, batW - 18, 14); _batterySubLabel.frame = CGRectMake(currentX + 18, padY + 14, batW - 18, 11); currentX += batW + 6.0f;
    ...
    if (showTemp) {
        CGFloat tempW = 60.0f; _tempIconLabel.frame = CGRectMake(currentX, padY + 2, 20, 22); _tempValueLabel.frame = CGRectMake(currentX + 22, padY, tempW - 22, 14); _tempSubLabel.frame = CGRectMake(currentX + 22, padY + 14, tempW - 22, 11); currentX += tempW + 6.0f;
    ...
    if (actualShowCurrent) {
        CGFloat curW = 58.0f; _currentIconLabel.frame = CGRectMake(currentX, padY + 3, 14, 22); _currentValueLabel.frame = CGRectMake(currentX + 16, padY, curW - 16, 14); _currentSubLabel.frame = CGRectMake(currentX + 16, padY + 14, curW - 16, 11); currentX += curW + 6.0f;
    }
    CGFloat finalW = MAX(currentX + 4.0f, 40.0f); CGFloat currentY = padY + 28.0f;
    ...
    if (isCharging) {
        ...
        if (![_marqueeLayer animationForKey:@"marqueeDashAnim"]) {
            CABasicAnimation *dashAnim = [CABasicAnimation animationWithKeyPath:@"lineDashPhase"]; dashAnim.fromValue = @(0); dashAnim.toValue = @(-40); dashAnim.duration = 0.8; dashAnim.repeatCount = HUGE_VALF; [_marqueeLayer addAnimation:dashAnim forKey:@"marqueeDashAnim"];
        }
...
- (void)updateDataWithCPU:(double)cpu cpuFreq:(double)cpuFreq fps:(double)fps battery:(NSInteger)battery temp:(double)temp current:(double)current isCharging:(BOOL)isCharging {
    ...
    if (!isCurrentlyChargeInhibited) {
        ...
        if (isCharging) statusColor = [UIColor colorWithRed:0.2f green:0.95f blue:0.5f alpha:1.0f]; else if (cpu >= 80.0 || temp >= 42.0) statusColor = [UIColor colorWithRed:1.0f green:0.23f blue:0.19f alpha:1.0f]; else if (temp >= 38.0) statusColor = [UIColor colorWithRed:1.0f green:0.62f blue:0.04f alpha:1.0f];
...
- (void)viewDidLoad {
    ...
    CGFloat margin = 16.0, screenW = [UIScreen mainScreen].bounds.size.width, screenH = [UIScreen mainScreen].bounds.size.height;
    CGFloat panelW = MIN(screenW - margin * 2, 420.0), panelH = MIN(screenH - margin * 4, 340.0);
    _blurEffectView = [[UIVisualEffectView alloc] initWithEffect:[UIBlurEffect effectWithStyle:UIBlurEffectStyleSystemMaterialDark]];
    _blurEffectView.frame = CGRectMake((screenW - panelW)/2.0, (screenH - panelH)/2.0, panelW, panelH); _blurEffectView.layer.cornerRadius = 18.0; _blurEffectView.layer.masksToBounds = YES; _blurEffectView.layer.borderWidth = 1.0; _blurEffectView.layer.borderColor = [UIColor colorWithWhite:1.0 alpha:0.18].CGColor;
    [self.view addSubview:_blurEffectView]; [_blurEffectView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:nil action:nil]];
    UIView *contentView = _blurEffectView.contentView;
    UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(16, 12, panelW - 60, 22)]; titleLabel.text = @"⚡ 系统与电池详细状态"; titleLabel.textColor = [UIColor whiteColor]; titleLabel.font = [UIFont systemFontOfSize:15 weight:UIFontWeightBold]; [contentView addSubview:titleLabel];
    UIButton *closeBtn = [UIButton buttonWithType:UIButtonTypeCustom]; closeBtn.frame = CGRectMake(panelW - 38, 10, 26, 26); [closeBtn setTitle:@"✕" forState:UIControlStateNormal]; [closeBtn setTitleColor:[UIColor colorWithWhite:0.8 alpha:1.0] forState:UIControlStateNormal]; closeBtn.titleLabel.font = [UIFont systemFontOfSize:14 weight:UIFontWeightBold]; [closeBtn addTarget:self action:@selector(closeDetailView) forControlEvents:UIControlEventTouchUpInside]; [contentView addSubview:closeBtn];
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, 40, panelW, 0.5)]; line.backgroundColor = [UIColor colorWithWhite:1.0 alpha:0.15]; [contentView addSubview:line];
    CGFloat colW = (panelW - 20) / 2.0, startY = 46.0, rowH = 22.0;
...
- (UILabel *)createRowWithTitle:(NSString *)title x:(CGFloat)x y:(CGFloat)y width:(CGFloat)width parent:(UIView *)parent {
    UILabel *keyLbl = [[UILabel alloc] initWithFrame:CGRectMake(x, y, width * 0.46, 20)]; keyLbl.text = [NSString stringWithFormat:@"%@:", title]; keyLbl.textColor = [UIColor colorWithWhite:0.75 alpha:1.0]; keyLbl.font = [UIFont systemFontOfSize:10.5 weight:UIFontWeightMedium]; keyLbl.adjustsFontSizeToFitWidth = YES; [parent addSubview:keyLbl];
    UILabel *valLbl = [[UILabel alloc] initWithFrame:CGRectMake(x + width * 0.46, y, width * 0.52, 20)]; valLbl.textColor = [UIColor whiteColor]; valLbl.font = [UIFont monospacedDigitSystemFontOfSize:10.5 weight:UIFontWeightBold]; valLbl.adjustsFontSizeToFitWidth = YES; valLbl.minimumScaleFactor = 0.5; [parent addSubview:valLbl];
    return valLbl;
}
...
- (void)refreshAllDetailData {
    ...
    [UIDevice currentDevice].batteryMonitoringEnabled = YES; NSInteger batPercent = (NSInteger)([UIDevice currentDevice].batteryLevel * 100); if (batPercent < 0) batPercent = 100;
    ...
    NSString *mfg = batInfo[@"Manufacturer"] ?: @"德赛"; if (mfg.length == 0) mfg = @"德赛";
    _labelsDict[@"电池健康程度"].text = [NSString stringWithFormat:@"%.0f%% %@", health, mfg]; _labelsDict[@"电池循环次数"].text = [NSString stringWithFormat:@"%ld次", (long)[batInfo[@"CycleCount"] integerValue]];
    BOOL charging = isChargingInternal(); NSInteger timeToFull = [batInfo[@"AvgTimeToFull"] integerValue];
    _labelsDict[@"电池预计充满"].text = (charging && timeToFull > 0 && timeToFull < 600) ? [NSString stringWithFormat:@"%ld小时 %ld分钟", (long)(timeToFull / 60), (long)(timeToFull % 60)] : (charging ? @"计算中..." : @"未在充电");
    _labelsDict[@"电池充电类型"].text = charging ? (batInfo[@"ChargerType"] ?: @"PD 快充") : @"未充电";
    double watts = [batInfo[@"Watts"] doubleValue]; _labelsDict[@"电池充电功率"].text = charging ? [NSString stringWithFormat:@"%.1fW", watts > 0 ? watts : 20.0] : @"0W";
    ...
    _labelsDict[@"电池当前电量"].text = [NSString stringWithFormat:@"%ld%%", (long)batPercent]; _labelsDict[@"电池设计容量"].text = [NSString stringWithFormat:@"%ldmAh", (long)designCap]; _labelsDict[@"电池实际容量"].text = [NSString stringWithFormat:@"%ldmAh", (long)maxCap]; _labelsDict[@"电池当前容量"].text = [NSString stringWithFormat:@"%ldmAh", (long)curCap];
    ...
    if (getifaddrs(&ifa_list) >= 0) {
        ...
        for (struct ifaddrs *ifa = ifa_list; ifa; ifa = ifa->ifa_next) {
            if (!ifa->ifa_addr || ifa->ifa_addr->sa_family != AF_LINK) continue; struct if_data *if_data = (struct if_data *)ifa->ifa_data; if (!if_data) continue;
            NSString *name = [NSString stringWithUTF8String:ifa->ifa_name];
            if ([name hasPrefix:@"en"]) { wifiIn += if_data->ifi_ibytes; wifiOut += if_data->ifi_obytes; } else if ([name hasPrefix:@"pdp_ip"] || [name hasPrefix:@"ipsec"] || [name hasPrefix:@"rmnet"] || [name hasPrefix:@"pdp"]) { cellIn += if_data->ifi_ibytes; cellOut += if_data->ifi_obytes; }
        }
        freeifaddrs(ifa_list); CFAbsoluteTime now = CFAbsoluteTimeGetCurrent(); double timeDiff = now - lastNetSpeedTime; if (timeDiff <= 0) timeDiff = 1.0;
        if (lastWifiInBytes > 0) { speedDownBytesPerSec = (uint64_t)((wifiIn - lastWifiInBytes + cellIn - lastCellInBytes) / timeDiff); speedUpBytesPerSec = (uint64_t)((wifiOut - lastWifiOutBytes + cellOut - lastCellOutBytes) / timeDiff); }
    ...
    _labelsDict[@"CPU信息"].text = [NSString stringWithFormat:@"%s %ld核心 %.0f%%", spec.chipName, (long)spec.cores, getSystemCPUUsage()];
    _labelsDict[@"CPU主频 / FPS"].text = [NSString stringWithFormat:@"%.0fMHz | %.0fFPS", getRealCPUFrequency(), [SBCPUFPSHelper sharedInstance].currentFPS];
    ...
    mach_port_t host_port = mach_host_self(); mach_msg_type_number_t host_size = sizeof(vm_statistics64_data_t) / sizeof(integer_t); vm_size_t pagesize; host_page_size(host_port, &pagesize); vm_statistics64_data_t vm_stat;
    if (host_statistics64(host_port, HOST_VM_INFO64, (host_info64_t)&vm_stat, &host_size) == KERN_SUCCESS) {
        uint64_t freeBytes = (uint64_t)(vm_stat.free_count + vm_stat.inactive_count + vm_stat.speculative_count) * (uint64_t)pagesize; _labelsDict[@"内存剩余"].text = [NSString stringWithFormat:@"%lluMB / %lluGB", freeBytes / (1024 * 1024), totalRAM_GB];
    }
    NSDictionary *fsAttrs = [[NSFileManager defaultManager] attributesOfFileSystemForPath:NSHomeDirectory() error:nil];
    _labelsDict[@"存储剩余"].text = [NSString stringWithFormat:@"%.2fGB / %lldGB", [fsAttrs[NSFileSystemFreeSize] longLongValue] / (1024.0 * 1024.0 * 1024.0), (int64_t)round((double)[fsAttrs[NSFileSystemSize] longLongValue] / (1024.0 * 1024.0 * 1024.0))];
    _labelsDict[@"蜂窝/WiFi"].text = [NSString stringWithFormat:@"%lluMB / %lluMB", lastCellInBytes / (1024 * 1024), lastWifiInBytes / (1024 * 1024)];
    if (_pedometer) {
        NSDate *now = [NSDate date]; NSCalendar *cal = [NSCalendar currentCalendar]; NSDateComponents *comp = [cal components:NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay fromDate:now]; NSDate *zeroDate = [cal dateFromComponents:comp];
    ...
    NSTimeInterval uptime = [[NSProcessInfo processInfo] systemUptime]; NSInteger days = (NSInteger)(uptime / 86400); NSInteger hours = (NSInteger)((uptime - days * 86400) / 3600); NSInteger mins = (NSInteger)((uptime - days * 86400 - hours * 3600) / 60);
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event { UIView *hitView = [super hitTest:point withEvent:event]; return hitView == self ? nil : hitView; }
@end
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (floatingView && !floatingView.hidden && floatingView.alpha > 0.01) { CGPoint p = [self convertPoint:point toView:floatingView]; if ([floatingView pointInside:p withEvent:event]) return floatingView; }
    return nil;
}
@end
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil]; NSArray *titles = @[@"80%", @"100%", @"120%", @"140%", @"160%", @"180%", @"200%"]; NSArray *values = @[@80, @100, @120, @140, @160, @180, @200];
    cell.textLabel.text = titles[indexPath.row]; if ([values[indexPath.row] doubleValue] == logoutCPUThreshold) cell.accessoryType = UITableViewCellAccessoryCheckmark;
    return cell;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil]; NSArray *titles = @[@"10 秒", @"30 秒", @"60 秒", @"120 秒", @"180 秒", @"300 秒", @"600 秒"]; NSArray *values = @[@10, @30, @60, @120, @180, @300, @600];
    cell.textLabel.text = titles[indexPath.row]; if ([values[indexPath.row] integerValue] == logoutDuration) cell.accessoryType = UITableViewCellAccessoryCheckmark;
    return cell;
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (section == 0) return 2; if (section == 1) return 3; if (section == 2) return 4; if (section == 3) return 3;
    if (section == 4) return 2; if (section == 5) return 5; if (section == 6) return 2; return 6;
}
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    if (section == 0) return @"📱 智能缩进与侧边吸附"; if (section == 1) return @"⚡ 自动控制与防护"; if (section == 2) return @"🔲 悬浮窗外观";
    if (section == 3) return @"🧠 智能选项"; if (section == 4) return @"🎮 性能与高刷锁定"; if (section == 5) return @"🌡️ Insulation (温控核心)"; 
    if (section == 6) return @"🔌 电池温控与断充"; return @"📍 位置与显示";
}
...
- (void)changeChargeTempSlider:(UISlider *)slider { smartChargeLimitTemp = slider.value; SavePreferencesAndNotify(); [self.tableView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:1 inSection:6]] withRowAnimation:UITableViewRowAnimationNone]; }
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        else if (indexPath.row == 1) { cell.textLabel.text = @"断充温度阈值"; UISlider *slider = [[UISlider alloc] initWithFrame:CGRectMake(0,0,130,30)]; slider.minimumValue = 30.0; slider.maximumValue = 45.0; slider.value = smartChargeLimitTemp; [slider addTarget:self action:@selector(changeChargeTempSlider:) forControlEvents:UIControlEventValueChanged]; cell.accessoryView = slider; cell.detailTextLabel.text = [NSString stringWithFormat:@"%.1f°C", smartChargeLimitTemp]; }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0 && indexPath.row == 1) {
        ...
        dockMode = (dockMode + 1) % 5; SavePreferencesAndNotify(); [tableView reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationNone];
        ...
        for (NSInteger i = 0; i < titles.count; i++) [alert addAction:[UIAlertAction actionWithTitle:titles[i] style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) { insulationCpuMode = i; SavePreferencesAndNotify(); [self.tableView reloadData]; }]];
        [alert addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:nil]]; [self presentViewController:alert animated:YES completion:nil];
    }
}
...
static void registerV160Observers(void) {
    [[NSNotificationCenter defaultCenter] addObserverForName:UIDeviceOrientationDidChangeNotification object:nil queue:NSOperationQueue.mainQueue usingBlock:^(NSNotification *n) { if (cpuWindow && floatingView) updateFloatingSize(); }];
    [[NSNotificationCenter defaultCenter] addObserverForName:UIKeyboardWillShowNotification object:nil queue:NSOperationQueue.mainQueue usingBlock:^(NSNotification *n) {
        ...
        if (cpuWindow && floatingView) {
            ...
            CGFloat keyboardHeight = endFrameValue ? MIN(320.0, [endFrameValue CGRectValue].size.height) : 220.0;
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:UIKeyboardWillHideNotification object:nil queue:NSOperationQueue.mainQueue usingBlock:^(NSNotification *n) {
        if (!settingsShowing && !detailShowing && keyboardMoved && floatingView) { [UIView animateWithDuration:0.25 animations:^{ floatingView.frame = keyboardBeforeFrame; }]; keyboardMoved = NO; }
    }];
}
...
- (NSProcessInfoThermalState)thermalState {
    if (insulationCpuMode == 2) return NSProcessInfoThermalStateNominal; 
    else if (insulationCpuMode == 1) return NSProcessInfoThermalStateCritical; 
    return %orig;
}
- (BOOL)isLowPowerModeEnabled { return insulationCpuMode == 1 ? YES : %orig; }
%end
%hook SBBacklightController
- (void)setThermalWarningState:(NSInteger)state { %orig(insulationDimmingEnable ? state : 0); }
- (void)_updateBrightnessForSunlightLoad:(BOOL)arg1 { %orig(insulationLockSunlight ? YES : arg1); }
%end
%hook SBThermalController
- (void)showThermalAlertIfNecessary { if (!insulationDisableThermometer) %orig; }
- (BOOL)isThermalBlocked { return insulationDisableThermometer ? NO : %orig; }
...
- (void)pocketStateDidChange:(NSInteger)state { %orig(insulationDisablePocketTemp ? 0 : state); }
%end

// ✅ [完美解决核心]：适配 iOS 15 Scene 生命周期的启动 Hook
%hook SpringBoard

- (void)applicationDidFinishLaunching:(id)application {
    %orig;
    
    // 方案一：深层监听 iOS 15 桌面的场景激活通知，100%保证能成功抓取可用 Scene 并展示悬浮窗
    ...
    // 方案二：应对无需硬重启的注销环境 (Respring)，此时 Scene 可能已经激活
    for (UIScene *scene in [UIApplication sharedApplication].connectedScenes) {
...
%ctor {
    ...
    // 这里彻底移除了原来暴力的 dispatch_after 等待代码，防黑屏和不显示。
}
